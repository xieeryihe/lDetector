...
你的任务是已知店铺q和店铺类型q_type, 判断用户构建的搜索关键词key_word是否合理,合理的标准是能够完成对店铺q进行搜索，并且符合普通用户表达习惯。
judgement是你的判断结果，我将对你的判断结果进行解析，请严格按照要求输出，key_word合理输出“yes”,不合理输出“no”。
以下是几个合理的keyword，可供参考。
正例一 q="那时新疆·车师（大宁音乐广场店）"，q_type='新疆菜' ，key_word='大宁 新疆菜'，judgement='yes'
正例二 q="涟泉小町·日式SPA（梅陇店）"，q_type='按摩/足疗'， key_word='做SPA'，judgement='yes'
正例三 q="蕃茄田艺术（宝山高境中心）"，q_type='美术'， key_word='画画 附近'，judgement='yes'
以下是几个不合理的keyword,可供参考。
反例一 q="小月半美甲美睫"，q_type='美甲' ，key_word='睫 小月半美甲美'，judgement='no'
这个keyword不合理的原因是“睫”不是可以售卖的商品或服务，因为断句错误而不符合普通用户表达习惯
反例二 q="澜斐儿（田图苑店）"，q_type='美甲'， key_word='美甲公众号'，judgement='no'
这个keyword不合理的原因是“美甲公众号”不是可以售卖的商品或服务，不符合普通用户表达习惯，并且不能对店铺q"澜斐儿（田图苑店）"完成搜索
反例三 q="东尚造型（永清路店）"，q_type='美发'， key_word='东尚附近剪头发'，judgement='no'
这个例子错误的原因是“东尚”是店名，不是地名，因此“东尚附近”不符合普通用户表达习惯，不是合理的搜索关键词。
反例四 q="海底捞（五角场合生汇店）"，q_type='火锅'， key_word='合生汇'，judgement='no'
...
def get_response(prompt):
    ...
    value_str = ""
    try:
        response = requests.post(api_url, headers=headers, data=data_json, timeout=20)  # 超时
...
def main():
    ...
    origin_excel = "chatglm2-6b.xlsx"
    target_excel = "chatglm2-6b_check.xlsx"
